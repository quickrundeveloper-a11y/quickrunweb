...
export default function DebitCardPage() {
  ...
  function fallbackCheck(value: string) {
    const firstOne = value.slice(0, 1);
    const firstTwo = value.slice(0, 2);
    const firstFour = value.slice(0, 4);

    let brand = "";

    if (firstOne === "4") brand = "Visa";
    else if ((parseInt(firstTwo) >= 51 && parseInt(firstTwo) <= 55) || (parseInt(firstFour) >= 2221 && parseInt(firstFour) <= 2720))
      brand = "Mastercard";
    else if (firstTwo === "60" || firstTwo === "65" || firstFour === "5085")
      brand = "RuPay";
    ...
    if (brand === "Visa" || brand === "Mastercard" || brand === "Discover") {
      setCardError(""); 
    } else if (brand === "RuPay") {
      setCardError("Debit cards are not allowed on this page.");
    } else {
    ...
    <div className="bg-gradient-to-br from-white  flex items-center justify-center">
        ...
        <div className="p-10 relative">
          ...
          <div className="mb-6">
            ...
            <div className={`flex items-center justify-between bg-gray-100 p-4 rounded-xl border ${cardError ? "border-red-500" : "border-gray-300"}`}>
              <input
                type="text"
                    ...
                    fetch(`/api/binlookup?bin=${bin}`)
                      ...
                      .then(data => {
                        const brand = data.scheme
                          ? data.scheme.charAt(0).toUpperCase() + data.scheme.slice(1)
                          : "";
